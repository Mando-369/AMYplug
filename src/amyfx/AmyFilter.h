...
public:
    ...
    void process(float* block, int n, float cutoffHz, float resonance, int type)
    {
        ...
        float coeffs[5];
        if (type == LPF || type == LPF24) biquad::genLpf(coeffs, ratio, resonance);
        else if (type == BPF)             biquad::genBpf(coeffs, ratio, resonance);
        else if (type == HPF)             biquad::genHpf(coeffs, ratio, resonance);
        else return;
